# MySQL Workbench

> MySQL Workbench el administrador de servidores de base de datos oficial para trabajar con MySQL Server
> Como el servidor de base de datos no tiene interfaz gráfica amigable, vamos a utilizar esta herramienta
> Vez que abramos la herramienta, debemos conectarnos al servidor

> Vamos a escribir código SQL en la ventana de código
> luego cuando lo ejecutemos veremos el resultado en el panel "Output"

> Para ejecutar los comandos tenemos un icono y también un atajo de teclado
1. el icono es el "rayito" ⚡ que está justo arriba de la paleta de código
2. El atajo de teclado es CTRL + INTRO (1 comando)
3. El atajo de teclado es CTRL + SHIFT + INTRO (más comando)
